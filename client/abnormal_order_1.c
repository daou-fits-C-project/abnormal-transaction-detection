...
void  make_abnormal_order_msg_1(char* buffer, size_t bufsize, int client_id) {
    ...
    int amount = 1;                                      // 10 ~ 90
    const char* type = random_order_type_1();                                  // BUY or SELL
...
DWORD WINAPI client_thread_1(LPVOID arg) {
...
void run_abnormal_order_1() {
    ...
    printf("특정 주식에 대한 비정상 거래 시나리오 시작 >>\n");
    Sleep(2000);
    ...
    for (int j = 0; j < 2; j++)
    {
        for (int i = 0; i < MULTI_CLIENT; i++) {
            client_ids[i] = i;
            threads[i] = CreateThread(NULL, 0, client_thread_1, &client_ids[i], 0, NULL);
            Sleep(1000);
        }
    }
