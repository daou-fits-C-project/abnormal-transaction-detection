...
const char* random_order_type_2() {
...
void make_abnormal_order_msg_2(char* buffer, size_t bufsize, int client_id) {
    ...
    int amount = 400;                                      // 10 ~ 90
    const char* type = random_order_type_2();                                  // BUY or SELL
    int account_id = 1111;
...
DWORD WINAPI client_thread_2(LPVOID arg) {
...
void run_abnormal_order_2() {
    ...
    printf("특정 계좌에 대한 비정상 거래량 시나리오 시작 >>\n");
    Sleep(2000);

    // 멀티 클라이언트 생성
    for (int i = 0; i < CLIENT; i++) {
        client_ids[i] = i;
        threads[i] = CreateThread(NULL, 0, client_thread_2, &client_ids[i], 0, NULL);
        Sleep(1000);
    }
